...
>
> - **Top-level files** (loaded directly by nora-fleet) resolve include paths relative to the **root folder**
...
```bash
> python -m nora_fleet.service.main_loop.server_main_loop --port 30011
...
For more details, please check the [Agent Manifest HOCON File Reference](
    https://github.com/nvsinha/nora-fleet/blob/main/docs/manifest_hocon_reference.md) documentation.
...
For a full description of the fields, please refer to the [Agent Network HOCON File Reference](
    https://github.com/nvsinha/nora-fleet/blob/main/docs/agent_hocon_reference.md) documentation.
...
Nora Fleet includes several predefined LLM providers and models. To use one of these, set the `model_name` key to
...
[default LLM info file](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/internals/run_context/langchain/llms/default_llm_info.hocon).
...
> `model_name` in Nora Fleet. If the `max_tokens` value in your `llm_config` exceeds the actual limit of the model
...
1. The `langchain-google-genai` package is already included with nora-studio,
...
> [nora-fleet-gemini-example](https://github.com/subhadeep-banerjee/nora-fleet-gemini-example) repository.
...
Nora Fleet's default Gemini model definitions — you must use the `class` key to instantiate
...
This guide walks you through how to use a locally running LLM via [Ollama](https://github.com/ollama/ollama) in nora-fleet.
...
See [agent network documentation](https://github.com/nvsinha/nora-fleet/blob/main/docs/agent_hocon_reference.md#max_execution_seconds)
for more details.

<!-- pyml disable line-length -->
Note that if your ollama model is not listed in [default_LLM_info.hocon file](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/internals/run_context/langchain/llms/default_llm_info.hocon#L1005) it might throw an 404 error, in which case explicitly adding the class should help.
<!-- pyml enable line-length -->
...
For more information about using unlisted ollama models with nora-fleet, please refer to the [agent hocon reference](https://github.com/nvsinha/nora-fleet/blob/main/docs/agent_hocon_reference.md#model_name).
...
Alternatively, you can change the system-wide default model used by Nora Fleet by modifying the `default_model_name`
value in the
<!-- pyml disable line-length -->
[default LLM info file](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/internals/run_context/langchain/llms/default_llm_info.hocon#L1005).
<!-- pyml enable line-length -->
...
> python -m nora_studio run
> ```

### See also

For a full description of `llm_config`, please refer to the [LLM config](
    https://github.com/nvsinha/nora-fleet/blob/main/docs/agent_hocon_reference.md#llm_config) documentation.

## LLM Fallbacks

Nora Fleet supports LLM fallbacks, which allow you to specify a list of LLMs to use in case the primary LLM fails.
...
- Implementing a custom factory
    ...
    - Your factory must subclass [`LangChainLlmFactory`](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/internals/run_context/langchain/llms/langchain_llm_factory.py)
    ...
    <!-- markdownlint-disable MD013 -->
    See
    [`StandardLangChainLlmFactory`](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/internals/run_context/langchain/llms/standard_langchain_llm_factory.py)
...
For more information on llm info file, please see [LLM Info HOCON File Reference](
    https://github.com/nvsinha/nora-fleet/blob/main/docs/llm_info_hocon_reference.md) documentation.
...
[music_nerd_pro](https://github.com/nvsinha/nora-studio/blob/main/docs/examples.md#music-nerd-pro) is a
...
[intranet_agents_with_tools](
    https://github.com/nvsinha/nora-studio/blob/main/docs/examples.md#intranet-agents-with-tools) is a
...
is given in here [math_guy_passthrough.hocon](
    https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/registries/math_guy_passthrough.hocon#L54)

For a full reference, please check the [nora-fleet documentation](https://github.com/nvsinha/nora-fleet/blob/main/docs/agent_hocon_reference.md#allow)
...
For more information on toolbox, please see [Toolbox Info HOCON File Reference](
    https://github.com/nvsinha/nora-fleet/blob/main/docs/toolbox_info_hocon_reference.md) documentation.
...
2. `MCP_SERVERS_INFO_FILE` Environment Variable
   ...
   - For example, see [mcp_info.hocon](../nora_studio/mcp/mcp_info.hocon)
...
3. OAuth bearer-token flow (client-driven)
    ...
    [nora_flow](https://github.com/nvsinha/nora-flow) — obtain the token and inject it into
   ...
   - For a complete example see [you_search.hocon](../registries/tools/you_search.hocon); for the client
    side (Connectors tab, token storage, redirect URI, troubleshooting) see nora_flow's
    [MCP OAuth Connectors guide](https://github.com/nvsinha/nora-flow/blob/main/docs/MCP_OAUTH.md).
...
in the Nora Fleet server environment):
...
Each individual agent in Nora Fleet runs its own internal LangGraph control loop — a state machine with three
...
- **`chat_history`** — a list of AI and human messages accumulated during the session. See
  [nora-fleet summarization middleware](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/middleware/nora_fleet_summarization_middleware.py)
  for an example.
...
The client and server logs will be saved to `logs/nora_flow.log` and `logs/server.log` respectively.
...
- Any updates to console logs can be managed via this plugin at `nora_studio/plugins/log_bridge/`.
- Use the `log_cfg` dict located at `nora_studio/plugins/log_bridge/process_log_bridge.py`
  to configure the formatting of logs.

## Debugging

1. To debug your code, set up your environment per these [instructions](https://github.com/nvsinha/nora-studio).
...
`music_nerd_pro`'s coded tool Python file (E.g., to the first line of `invoke` method in `Accountant` [class](https://github.com/nvsinha/nora-studio/blob/main/coded_tools/basic/accountant.py)
...
3. Start the client and server via `python -m nora_studio run`,
...
Furthermore, it is also possible to reference agents on other nora-fleet servers by using a URL as a tool reference.
...
Plugins are a way to extend the functionality of a Nora Fleet server largely for
deployment-related use-cases. Note that plugins are never required for Nora Fleet to function.
...
```bash
python -m pytest tests/ -v --cov=coded_tools --cov=nora_studio -m "not integration"
```
...
[test case HOCON reference](https://github.com/nvsinha/nora-fleet/blob/main/docs/test_case_hocon_reference.md).
...
[Data-Driven Test Case HOCON File Reference](https://github.com/nvsinha/nora-fleet/blob/main/docs/test_case_hocon_reference.md)
in the nora-fleet repository.
