...
Run `nora-studio check-llm-keys` to validate your API keys using a three-tier system. You can
specify the tier level with `--tier`:

- `nora-studio check-llm-keys` or `nora-studio check-llm-keys --tier 3` — Run all three tiers (default)
- `nora-studio check-llm-keys --tier 1` — Run only Tier 1
- `nora-studio check-llm-keys --tier 2` — Run Tiers 1 and 2
...
> [`nora_studio/commands/check_llm_keys.py`](../nora_studio/commands/check_llm_keys.py).
