# Nora Studio – Getting Started

Welcome to the **Nora Studio** tutorial. In this guide, we will walk you through the process of
setting up a **Data-Driven Multi-Agent Network** using the `nora-fleet` library, managing it via web-based clients,
...
* [Nora Studio – Getting Started](#nora-studio--getting-started)
    ...
    * [7. How to use tools in Nora Fleet](#7-how-to-use-tools-in-nora-fleet)
    ...
    * [11. Key Aspects of Nora Studio](#11-key-aspects-of-nora-studio)
...
**Nora Fleet** is a multi-agent orchestration library that enables you to build **data-driven agent networks**. These
...
[nora_flow](https://github.com/nvsinha/nora-flow) or [MAUI](https://github.com/nvsinha/nora-fleet-ui).
...
* `nora_studio/commands/run.py`: A starter script to run the server and the web client.
...
git clone https://github.com/nvsinha/nora-studio
cd nora-studio
...
In Nora Studio, Agents may declare a function with defined parameters. Other agents can call this
...
Because **Nora Studio** uses `nora-fleet`, it is LLM-agnostic, you can switch to different models or
...
* [LLM Info HOCON Reference Guide](https://github.com/nvsinha/nora-fleet/blob/main/docs/llm_info_hocon_reference.md)
* [Agent HOCON Reference Guide](https://github.com/nvsinha/nora-fleet/blob/main/docs/agent_hocon_reference.md)
...
## 7. How to use tools in Nora Fleet

Nora Fleet supports two types of tools that agents can call during execution:

1. **Custom Tools** – Built using Nora Fleet’s `CodedTool` interface. These allow you to define your own logic in Python
...
In Nora Fleet, custom tool integration is done through **Coded Tools** — Python class that implement the `CodedTool` interface.
...
Nora Fleet supports the following data types:  
...
* Implement either `invoke` or `async_invoke`
  > Nora Fleet prefers `async_invoke`, and falls back to `invoke` if unavailable
...
Nora Fleet includes a powerful **Toolbox** system that allows you to reuse existing tools across agent networks without
...
This [file](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/internals/run_context/langchain/toolbox/toolbox_info.hocon)
...
> **Note**: The asynchronous variants (`abefore_agent`, etc.) are preferred in the Nora Fleet server environment.
...
When you are running the server in the foreground (via `python -m nora_studio run`), simply press:

* `CTRL + C` on Windows/Mac/Linux terminals
* This will terminate both the nora_flow client and the `nora_fleet` server gracefully.

If you launched them separately, you would stop each process individually (again by `CTRL + C` or sending a kill signal).

---

## 11. Key Aspects of Nora Studio
...
tools. We hope you find Nora Studio intuitive, powerful, and adaptable to your needs.
