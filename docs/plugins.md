...
Plugins are a way to extend the functionality of a Nora Fleet server largely for deployment-related use-cases.
Note that plugins are never required for Nora Fleet to function.
...
All plugins extend the `BasePlugin` class in `nora_studio/plugins/base_plugin.py` and are registered in
...
**Runner process** (`nora_studio/commands/run.py`) -- manages subprocesses:
...
**Server process** (`nora_studio/runner/nora_fleet_server_wrapper.py`) -- in-process server:
...
See [`BasePlugin`](../nora_studio/interfaces/base_plugin.py) for the full interface and
[`PhoenixPlugin`](../nora_studio/plugins/phoenix/phoenix_plugin.py) for a real-world implementation.
...
[Open FGA](../nora_studio/plugins/openfga/README.md) is a plugin that extends the authorization capabilities
of a Nora Fleet server using a free and open source Open FGA server to do Relation-Based Access Control (ReBAC)
authorization.

## Logging

Logging plugins enhance the console and file logging experience for Nora Studio,
providing structured, human-readable output from server and client subprocesses.

### Log Bridge

The [Log Bridge plugin](../nora_studio/plugins/log_bridge/README.md) provides Rich-based structured logging for
Nora Studio, replacing raw subprocess output with colored, pretty-printed, and severity-aware
...
The [Arize Phoenix plugin](../nora_studio/plugins/phoenix/README.md) integrates
[Arize Phoenix](https://arize.com/phoenix/) for AI observability in Nora Studio,
providing comprehensive monitoring and analysis of LLM interactions.

### Langfuse

The [Langfuse integration](../nora_studio/plugins/langfuse/README.md) provides
[Langfuse](https://langfuse.com/) observability for Nora Studio: trace collection, cost tracking, and
performance metrics for LLM interactions, with support for both cloud and self-hosted Langfuse instances.
Langfuse tracing is built into Nora Fleet itself — no plugin class is required. To use it, install the
optional dependency (`pip install -r nora_studio/plugins/langfuse/requirements.txt`) and set
`LANGFUSE_ENABLED=true` along with `LANGFUSE_SECRET_KEY`, `LANGFUSE_PUBLIC_KEY`, and (optionally)
`LANGFUSE_HOST` in your `.env` file. See the [README](../nora_studio/plugins/langfuse/README.md) for
the full configuration reference and troubleshooting.

### LangSmith

[LangSmith](../nora_studio/plugins/langsmith/README.md) is LangChain's built-in observability platform.
Since Nora Fleet uses LangChain internally, LangSmith tracing works out of the box with no plugin required — just set
