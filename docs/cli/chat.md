...
Chat with an agent network directly from the command line, without starting nora-fleet and nora_flow servers.
This command delegates to nora-fleet's `AgentCli` (also available as `python -m nora_fleet.client.agent_cli`).
...
Under the hood this command delegates to nora-fleet's `AgentCli`, so it stays in sync with the library's capabilities.

## Usage

```bash
# Interactive chat with an agent (direct/library connection, no server needed)
nora chat basic/music_nerd

# One-shot mode: send a prompt from a file and exit
nora chat basic/music_nerd --one-shot --first_prompt_file prompt.txt

# Connect to a running nora-fleet server
nora chat basic/music_nerd --connection http --host localhost --port 8080
...
nora chat basic/music_nerd --connectivity
```
...
| `--host` | Hostname of the nora-fleet server (for http/https connections). |
| `--port` | Port of the nora-fleet server. |
| `--one-shot` | Send one prompt and exit (non-interactive mode). |
| `--list` | List all available agents and exit (ignores AGENT argument). |

## Advanced options

These options are forwarded directly to `AgentCli` and do not appear in `nora chat --help`.
Use `python -m nora_fleet.client.agent_cli --help` for the full reference.
...
nora chat music_nerd --one-shot --first_prompt_file /tmp/prompt.txt
```

### Remote server connection

```bash
nora chat basic/music_nerd --connection https --host my-server.example.com --port 443
```
