...
Validates the *structure* of an agent network HOCON file against nora-fleet's agent network validation rules. Use it to
...
Under the hood this command delegates to nora-fleet's `HoconValidatorCli` (the tool also available as
`python -m nora_fleet.client.hocon_validator_cli`), so it stays in sync with the library's validation rules.

## Usage

```bash
# Validate an agent network HOCON file
nora-studio validate registries/music_nerd.hocon

# Same command via the shorter alias
nora validate registries/music_nerd.hocon

# Print an agent network summary when validation passes
nora validate registries/music_nerd.hocon --verbose
```
