...
modes: discovery-driven (pull networks from the installed nora-studio
...
```bash
nora import basic                          # one group
nora import industry experimental          # multiple groups
nora import all                            # everything
nora import music_nerd                     # one network (any group)
nora import basic agent_network_designer   # mix
```
...
```bash
nora import path/to/network.hocon            # self-contained single HOCON
nora import music_nerd.hocon                 # bare name resolved in the current directory
nora import path/to/bundle.zip               # network + dependencies
nora import a.hocon path/to/b.zip            # multiple files in one call
nora import path/to/bundle.zip --force       # overwrite existing files
```
...
[`nora export`](./export.md) to ship a network between projects.
...
`expertise_scoping_instructions.hocon`) are scaffolded by `nora init`; the importer copies them as a safety net if
missing.
...
Run from a project initialized with `nora init` (must contain `registries/manifest.hocon`). nora-studio must be importable.
