...
`nora import -f` consumes. The output format is auto-detected from the network's
dependencies: a self-contained `.hocon` if there are none, a `.zip` carrying the
network plus its deps otherwise.

Pairs round-trip with [`nora import -f`](./import.md#from-a-local-file--f). Build a
network in one project, run `nora export`, ship the file, run `nora import -f` on
the receiver.
...
```bash
nora export music_nerd                          # → music_nerd.{hocon|zip} in cwd
nora export basic/music_nerd                    # grouped network
nora export agent_network_designer -o /tmp/and.zip
nora export music_nerd -o /tmp/mn.hocon         # explicit single-HOCON output
```
...
| `export_nora_studio_version` | `0.3.3` | Version of nora-studio that produced the bundle |
...
Run from a project initialized with `nora init` (must contain
