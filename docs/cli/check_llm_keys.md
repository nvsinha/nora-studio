...
`nora-studio check-llm-keys` runs three progressively deeper tiers of validation on LLM API keys
...
nora-studio check-llm-keys

# Tier 1 — placeholder detection only
nora-studio check-llm-keys --tier 1

# Tier 2 — placeholder + format checks (no network calls)
nora-studio check-llm-keys --tier 2
```
