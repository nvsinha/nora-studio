...
- `python -m nora_studio run` automatically points `AGENT_TOOLBOX_INFO_FILE` at this repo's
  [toolbox_info.hocon](../../../nora_studio/toolbox/toolbox_info.hocon), where `web_fetch` is defined. If you
...
- A shared toolbox coded tool (`nora_studio.coded_tools.web_fetch.WebFetch`) that fetches one URL and returns
