...
- Demonstrates the functionality of the generated network by calling the `NoraFlowSelenium` coded tool.
...
**Additional setup for NoraFlowSelenium:**

- Start the `nora_flow` service in a separate terminal.
- Use **non-default** ports for nora-fleet HTTP and nora_flow. The default ports are 8080 for HTTP and
4173 for nora_flow. For example, you can run:

    ```bash
    python -m nora_studio run --server-http-port 8081 --nora_flow-port 4174
    ```
...
3. **agent_network_tester**
   - Calls the `NoraFlowSelenium` coded tool to launch the agent network in a Chrome instance.
