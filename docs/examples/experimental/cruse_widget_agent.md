...
The CRUSE Interface is available at `/cruse` in nora_flow and displays these widgets inline within the chat conversation.
...
NORA_FLOW_CRUSE_WIDGET_AGENT_NAME=cruse_widget_agent
```

If `NORA_FLOW_PLUGIN_CRUSE=false`, the CRUSE interface is hidden and widgets don't appear.
