...
The **CRUSE Theme Agent** is a specialized agent designed to generate context-aware visual themes for the CRUSE (Context Reactive User Experience) Interface in nora_flow. It analyzes agent metadata to create dynamic or static background themes that match the agent's domain and purpose.
...
The CRUSE Interface is available at `/cruse` in nora_flow and renders these themes.
...
NORA_FLOW_PLUGIN_CRUSE=true

# Theme Agent Name - Customize if needed
NORA_FLOW_CRUSE_THEME_AGENT_NAME=cruse_theme_agent
```

If `NORA_FLOW_PLUGIN_CRUSE=false`, the CRUSE interface is hidden.
