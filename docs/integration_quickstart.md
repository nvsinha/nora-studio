# Nora Fleet Integration Quick Start

This tutorial will teach you how to use a nora-fleet agent network as a Python library using the "direct" session feature.
In this tutorial, we assume that we are working in a blank directory. If you've already created a nora-fleet project
with `nora init` command, you can directly skip to step 3 of this tutorial.

## Who is this for ?

This tutorial maybe useful, if you:

- Have already created an agent network in nora-studio and want to run it programmatically
- Want to integrate nora-fleet agents into your Python applications
...
2. Register your agent with nora-fleet
3. Invoke agents directly from Python code
4. Create reusable functions for agent interactions
5. Use HTTP requests to communicate with a nora-fleet server

## Prerequisites

Before starting, ensure you have:
- Setup your nora-fleet environment -> [README](../README.md)
- API keys for your chosen LLM provider (such as OpenAI for GPT-4o)
- An empty folder to work in

> **Tip:** If you just want to chat with an agent network without writing any Python, the `nora chat`
...
An agent in nora-fleet is defined using a HOCON configuration file. Let's create a simple
...
nora-fleet uses a `manifest.hocon` file to discover available agents. Let's create this manifest and register our agent.
...
- Setting `"music_nerd.hocon": true` tells nora-fleet to load this agent
...
```py
from nora_fleet.client.direct_agent_session_factory import DirectAgentSessionFactory
...
# Format it in the structure nora-fleet expects
request_payload = {
    "user_message": {
        "text": user_message,
    }
...
def invoke_agent(agent_name: str, user_text: str, sly_data=None):
    """
    Invoke a nora-fleet agent and return its response.
...
If you prefer running nora-fleet as a separate server process, you can interact with it
via HTTP requests instead of direct invocation.

### 5.1: Start the Server and Client

There are two options to start the nora-fleet server and nora_flow client.

If you've installed nora-studio package to your project:
...
This starts the nora-fleet HTTP API on `http://localhost:8080` by default (and also launches the
nora_flow UI on `http://localhost:4173/`). See the
[README](https://github.com/nvsinha/nora-studio/blob/main/README.md#install) and
[docs/cli.md](cli.md) for details, or use `nora run --server-only` to skip the UI.
...
[agent_cli](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/client/agent_cli.py)
...
**HOCON issues?**
- Use `nora validate path/to/hocon` to check that your hocon is valid
