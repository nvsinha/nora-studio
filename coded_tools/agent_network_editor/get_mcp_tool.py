...
# Path to the mcp_info.hocon shipped inside the nora_studio package.
...
class GetMcpTool(CodedTool):
    ...
    * sly_data["http_headers"] — per-conversation auth headers injected by
      an OAuth-capable client. nora_flow injects a fresh Authorization header
      for EVERY connected MCP server when the chat target is the Agent
      Network Designer (its NORA_FLOW_WAND_NAME), and the designer network
    ...
    # TODO: This duplicates NoraFleetRunner._resolve_mcp_info_file in
    # nora_studio/commands/run.py. Refactor so run.py calls this method
    # instead of maintaining its own copy.
    @staticmethod
    def get_mcp_info_file() -> str:
        """Resolve the mcp_info.hocon path at call time.

        Precedence (mirrors NoraFleetRunner._resolve_mcp_info_file):
          1. MCP_SERVERS_INFO_FILE env var (used verbatim if non-empty).
          2. <cwd>/mcp/mcp_info.hocon if it exists (what `init` scaffolds).
          3. The mcp_info.hocon bundled in the nora_studio package.
        """
    ...
    def _load_mcp_servers() -> McpServersLoad:
        ...
        try:
            ...
            # nora-fleet re-wraps HOCON parse errors as ValueError; OSError
    ...
    def sly_data_http_header_urls(sly_data: dict[str, Any] | None) -> list[str]:
        ...
        an OAuth-capable client injects (nora_flow injects one for every
    ...
    async def _fetch_sly_data_tool_descriptions(sly_data: dict[str, Any], exclude: set[str]) -> dict[str, str]:
        ...
        conversation's credentials (nora_flow refreshes them every message),
