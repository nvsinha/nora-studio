...
class GetSubnetwork(BranchActivation, CodedTool):
    ...
    def _manifest_update_period_seconds() -> float:
        ...
        :return: The manifest refresh period, read from the same
                ...
                nora-fleet server's own periodic manifest updates (<= 0
    ...
    def _load_subnetwork_names() -> list[str]:
        ...
        try:
            ...
            # Use nora-fleet's canonical manifest filters so we don't reimplement manifest semantics:
            ...
            # Derive external network names ("/<network_name>") via nora-fleet's
            ...
            # nora-fleet's restorer deliberately re-wraps HOCON parse errors
    ...
    async def get_subnetworks(self) -> dict[str, str]:
        ...
        # AttributeError raised INSIDE the nora-fleet accessors (e.g. version skew).
