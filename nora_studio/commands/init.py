...
"""Implementation of the `nora-studio init` command."""
...
from nora_studio.utils.cli_status import CliStatus
...
TEMPLATES_PACKAGE = "nora_studio.templates"
...
class InitCommand:  # pylint: disable=too-few-public-methods
    """Scaffold a starter nora-studio project in the current directory."""
    ...
    def run(self) -> None:
        ...
        # any files. Empty `{}` is a valid manifest — nora-fleet just sees no extra networks.
        ...
        # scaffolding them up front means `nora import <group>` works without surprises.
        for shared in SHARED_REGISTRY_INCLUDES:
            self._copy_template(shared, os.path.join("registries", shared), package="registries")
        self._copy_template("mcp_info.hocon", os.path.join("mcp", "mcp_info.hocon"), package="nora_studio.mcp")
    ...
    def _render_llm_config(providers: List[str]) -> str:
        ...
        ``registries/basic/music_nerd_llm_fallbacks.hocon``. The nora-fleet agent
    ...
    def _copy_template(self, template_name: str, dest_rel: str, package: str = TEMPLATES_PACKAGE) -> None:
        ...
        Args:
            ...
            package: Source package to read from. Defaults to nora_studio.templates.
        """
    ...
    def _print_next_steps(self) -> None:
        ...
        _console.print("  2. Start the server:  [bold red]nora-studio run[/bold red] or [bold red]nora run[/bold red]")
        _console.print("=" * 60, style="dim")
