...
"""Implementation of the `nora-studio internalize-agents` command.
...
class InternalizeAgentsCommand:
    ...
    def parse_hocon(path: Path) -> dict[str, Any]:
        """Parse a HOCON (or JSON) file via nora-fleet's AbstractAsyncConfigRestorer so that
    ...
    def internalize_externals(
        ...
        try:
            # Top-level `tools` is the list of agents in a nora-fleet network.
