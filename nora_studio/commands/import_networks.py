...
"""Import agent networks from the installed nora-studio package into the current project."""
...
from nora_studio.discovery.agent_network_registry import AgentNetworkRegistry
...
from nora_studio.importer.agent_network_importer import is_skippable_metadata
from nora_studio.utils.cli_prompt import CliPrompt
...
class ImportCommand:  # pylint: disable=too-few-public-methods
    """Run the `nora import` flow: discover, prompt, resolve dependencies, copy, update manifest."""
    ...
    def run(self) -> None:
        ...
        if not self._verify_project_initialized():
            print()
            CliStatus.err("Project not initialized. Run 'nora init' first.")
            print()
            sys.exit(1)
        ...
        # is the user-typed path. Same end behavior as `nora import <path>`.
        ...
        print("        - Run 'nora run' to start the server")
        print("        - If nora-fleet server is running, the manifest will auto-reload.")
        print()
