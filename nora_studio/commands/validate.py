...
"""Implementation of the `nora-studio validate` command.

Validates the structure of an agent network HOCON file by delegating to the
nora-fleet library's ``HoconValidatorCli`` (the tool also exposed as
``python -m nora_fleet.client.hocon_validator_cli``). Studio stays a thin
...
# Exit codes. nora-studio's main() treats code 2 as a clean "help" exit,
...
class ValidateCommand:  # pylint: disable=too-few-public-methods
    ...
    Thin wrapper around nora-fleet's ``HoconValidatorCli``. Returns exit code 0
