...
class AgentNetworkImporter:
    ...
    # alongside any imported network. (llm_config is generated fresh by `nora init`, not copied.)
    # Defined in nora_studio.utils.shared_registries so `nora init` scaffolds exactly the
    ...
    def _register_manifest_entry(self, result: ImportResult, registries_relative: str) -> None:
        ...
        crash nora-fleet at startup — its validator iterates the file expecting agent specs
    ...
    def _merge_mcp_from_source(self, mcp_urls: List[str], result: ImportResult) -> None:
        ...
        scaffolded one — same precedence as ``nora run`` uses to find the active config.
        """
