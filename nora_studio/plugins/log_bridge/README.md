...
This plugin provides a Rich-based structured logging bridge for Nora Studio, replacing raw subprocess output with colored, pretty-printed, and severity-aware console logs.
...
- **Non-invasive integration** — falls back to basic streaming when disabled; no changes to core Nora Fleet code
...
The `log_cfg` dictionary at the top of `nora_studio/plugins/log_bridge/process_log_bridge.py` controls formatting details. You can customize it without code changes by passing an override `config` dict when constructing `ProcessLogBridge`.
...
The Log Bridge is a self-contained plugin in `nora_studio/plugins/log_bridge/`. When enabled, it intercepts subprocess stdout/stderr streams and processes each line through a parsing pipeline before emitting formatted output to the console. When disabled, `nora_studio/commands/run.py` falls back to basic threaded log streaming.

### Key Components

#### ProcessLogBridge Class

The `ProcessLogBridge` class (in `nora_studio/plugins/log_bridge/process_log_bridge.py`) manages:
...
### Integration with nora_studio/commands/run.py

The runner (`nora_studio/commands/run.py`) integrates the Log Bridge in two places:
...
2. **Process attachment** — each launched subprocess (server, web client, nora_flow) is attached via `log_bridge.attach_process_logger(process, name, log_file)`, which spawns drain threads for both stdout and stderr
...
- **Brace-balanced JSON reassembly**: Handles JSON payloads that span multiple lines (common in Nora Fleet's structured logging) without requiring a fixed line count or delimiter
...
The Log Bridge is **enabled by default**. When you start Nora Studio:
...
When disabled, `nora_studio/commands/run.py` falls back to simple threaded output streaming without Rich formatting, JSON parsing, or per-process log files.
