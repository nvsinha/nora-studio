...
Langfuse tracing is built into nora-fleet itself — there is no plugin class to load.
...
```bash
pip install -r nora_studio/plugins/langfuse/requirements.txt
```

or, when installing the package:

```bash
pip install "nora-studio[langfuse]"
```
...
`CallbackHandler` is registered in the server process on top of nora-fleet's built-in
