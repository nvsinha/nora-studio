...
This plugin integrates a Nora Fleet server with an [OpenFGA](https://openfga.dev/) server for
...
is _not_ a responsibility within the core capabilities of a Nora Fleet server.

Most often a Nora Fleet server is a single component in a larger cluster deployment where
...
- **Container Based** - OpenFGA is a container image completely separate from the Nora Fleet server.
...
1. Install the Python requirements for the Nora Fleet OpenFGA Authorization plugin in your virtual environment.

```bash
pip install -r nora_studio/plugins/openfga/requirements.txt
```

2. We encapsulate the starting of a SQLite-based OpenFGA server in the following script:

```bash
./nora_studio/plugins/openfga/run_openfga_server.sh
```
...
enable the use of the OpenFGA server for authorization within your Nora Fleet server:

```bash
# What class nora-fleet server should use for authorization
export AGENT_AUTHORIZER=nora_fleet.service.authorization.openfga.open_fga_authorizer.OpenFgaAuthorizer
...
export FGA_POLICY_FILE=nora_studio/plugins/openfga/sample_authorization_model.json
...
4. In a shell with the same virtual env and environment variables set,
   ...
   This command is given from the nora-studio repo top-level directory.

```bash
python nora_studio/plugins/openfga/authorize.py
```
...
5. In a shell with the same virtual env and environment variables set,
   now run your nora-fleet server.
...
6. In yet another shell, try running a listing of the agents against your running Nora Fleet server
   with your existing user.

```bash
python -m nora_fleet.client.agent_cli --http --list
```
...
```bash
USER="bogus" python -m nora_fleet.client.agent_cli --http --list
```
...
```bash
sudo nora_studio/plugins/openfga/install_fga_cli.sh
```
...
```bash
    ...
    fga model validate --file nora_studio/plugins/openfga/sample_authorization_model.fga
```
...
```bash
    ...
    fga model transform --file nora_studio/plugins/openfga/sample_authorization_model.fga | python -m json.tool > nora_studio/plugins/openfga/sample_authorization_model.json
```
...
3. Re-run your OpenFGA and nora-fleet servers with the same env vars as described in previous sections
...
```bash
    fga model test --tests nora_studio/plugins/openfga/agent_network_access_test.fga.yml
```
...
- [The Nora Fleet Dockerfile](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/deploy/Dockerfile)
  has the documentation for each of the environment variables mentioned above.

- [The Nora Fleet Authorizer interface](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/service/authorization/interfaces/authorizer.py)
  contains documentation and examples for each of the methods of the Authorizer interface

- [The OpenFGA implementation of the Authorizer interface](https://github.com/nvsinha/nora-fleet/blob/main/nora_fleet/service/authorization/openfga/open_fga_authorizer.py)
