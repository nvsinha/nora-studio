...
Since Nora Fleet uses LangChain internally, LangSmith tracing works out of the box with no plugin required — just set environment variables.
...
That's it. All LangChain `Runnable.invoke()` / `.ainvoke()` calls (including those inside Nora Fleet's `RunContextRunnable`) are automatically traced.
