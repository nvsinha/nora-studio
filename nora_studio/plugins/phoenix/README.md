...
This plugin integrates [Arize Phoenix](https://arize.com/phoenix/) for AI observability in Nora Studio, providing comprehensive monitoring and analysis of LLM interactions.
...
- **Plugin-based architecture** for clean, non-invasive integration with Nora Studio
...
```bash
pip install -r nora_studio/plugins/phoenix/requirements.txt
```
...
**Note:** Phoenix is now integrated as a plugin that initializes when the Nora Fleet server starts. The plugin handles all instrumentation automatically when enabled.
...
| `OTEL_SERVICE_NAME` | `nora-fleet-demos` | Service name for OpenTelemetry |
...
Phoenix observability is implemented as a self-contained plugin following a refactoring in [PR #404](https://github.com/nvsinha/nora-studio/pull/404). The design prioritizes:

- **Non-invasive integration**: Phoenix initialization occurs only when the Nora Fleet server starts, not during pip operations or tests
- **Centralized logic**: All Phoenix-related code is consolidated in the `nora_studio/plugins/phoenix/` directory
...
The `PhoenixPlugin` class (in `nora_studio/plugins/phoenix/phoenix_plugin.py`) manages:
...
- **Separate requirements**: Phoenix dependencies are isolated in `nora_studio/plugins/phoenix/requirements.txt`, making it easy to install only when needed
...
- Process management could be consolidated across plugins (tracked in [issue #426](https://github.com/nvsinha/nora-studio/issues/426))
...
The plugin initializes when the Nora Fleet server starts, ensuring observability is available throughout your agent network's lifecycle.

### Viewing Traces

1. Start Nora Studio with Phoenix enabled
...
- [PR #404: Phoenix Plugin Refactoring](https://github.com/nvsinha/nora-studio/pull/404) - Major refactoring to plugin-based architecture
- [Issue #426: Consolidate process management across plugins](https://github.com/nvsinha/nora-studio/issues/426) - Future improvement for unified plugin lifecycle management
