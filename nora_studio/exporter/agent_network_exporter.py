...
"""Bundle an agent network from a project directory into the shape `nora import -f` consumes."""
...
from nora_studio.exporter.export_metadata import ExportMetadataStamper
from nora_studio.mcp.mcp_info_merger import McpInfoMerger
...
# `nora init` scaffolds <project>/mcp/mcp_info.hocon. We export from the project's copy when
# present, falling back to the studio package's bundled mcp_info.hocon — same precedence
# as the runtime resolver in NoraFleetRunner.
...
class AgentNetworkExporter:  # pylint: disable=too-few-public-methods
    ...
    Source of truth is the user's project (`project_dir`) — same layout as `nora import`
    ...
    def _add_filtered_mcp_info(
        ...
        yet (matches NoraFleetRunner._resolve_mcp_info_file precedence). If no source file is
    ...
    def _resolve_network(self, network: str) -> str:
        ...
        # Tolerate a repo-root-style `registries/...` prefix: `nora export
