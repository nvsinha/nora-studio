...
from tests.nora_studio.coded_tools.web_fetch.helpers import make_stream_session
...
class TestFetchPdf(TestCase):
    ...
    def test_returns_joined_page_text(self):
        ...
        with (
            ...
            patch("nora_studio.coded_tools.utils.pdf_utils.PdfReader", return_value=mock_reader),
        ):
...
class TestDownloadPdfBytes(TestCase):
    ...
    def test_streamed_body_over_limit_raises(self):
        ...
        with patch("nora_studio.coded_tools.web_fetch.MAX_RESPONSE_BYTES", 10):
            with self.assertRaises(ValueError) as ctx:
                self._call(session)
        self.assertIn("response_too_large", str(ctx.exception))
