...
class TestInteractivePicker:
    """`nora export` with no positional arg drops into a single-select picker over the project manifest."""
    ...
    def test_picker_returns_chosen_path(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("nora_studio.commands.export_networks.questionary.select", fake_select)
    ...
    def test_picker_handles_keyboard_interrupt(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "nora_studio.commands.export_networks.questionary.select",
...
class TestRunCancellation:  # pylint: disable=too-few-public-methods
    ...
    def test_run_exits_clean_when_picker_cancelled(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(
            "nora_studio.commands.export_networks.ExportCommand._prompt_for_network",
            lambda self: None,
        )
        ...
        monkeypatch.setattr("nora_studio.commands.export_networks.AgentNetworkExporter.export", boom)
