...
"""Tests for the `nora-studio init` command."""
...
from nora_studio.commands.init import InitCommand
from nora_studio.importer.agent_network_importer import AgentNetworkImporter
from nora_studio.utils.shared_registries import SHARED_REGISTRY_INCLUDES
...
class TestLlmConfigRendering:
    ...
    def test_empty_providers_raises(self) -> None:
        ...
        An empty ``fallbacks`` list is syntactically valid HOCON but the nora-fleet
...
class TestRunFlow:
    ...
    def _assert_matches_template(
        ...
        template_name: str,
        dest_rel: str,
        package: str = "nora_studio.templates",
    ) -> None:
    ...
    def test_music_nerd_sourced_from_templates(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        """music_nerd.hocon should be copied from nora_studio.templates."""
        self._run_init(tmp_path, monkeypatch)
    ...
    def test_manifest_sourced_from_templates(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        """manifest.hocon should be copied from nora_studio.templates."""
        self._run_init(tmp_path, monkeypatch)
    ...
    def test_mcp_info_sourced_from_mcp_package(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        """mcp_info.hocon should be copied from nora_studio.mcp (the same file run.py uses)."""
        self._run_init(tmp_path, monkeypatch)
        self._assert_matches_template(tmp_path, "mcp_info.hocon", "mcp/mcp_info.hocon", "nora_studio.mcp")

    def test_plugins_sourced_from_templates(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        """plugins.hocon should be copied from nora_studio.templates."""
        self._run_init(tmp_path, monkeypatch)
...
class TestTemplateSync:
    ...
    def _assert_template_matches_source(template_name: str, source_rel: str) -> None:
        ...
        template = (importlib.resources.files("nora_studio.templates") / template_name).read_bytes()
...
class TestSharedRegistryIncludes:
    """Guard the single shared-includes list that `nora init` and `nora import` both consume."""
    ...
    def test_every_shared_include_exists_in_the_packaged_registries(self) -> None:
        """Each name must resolve to a real file, or `nora init` silently scaffolds nothing.

        `_copy_template` reads through importlib.resources, so a typo or a renamed fragment
        surfaces only at scaffold time — and `nora import` degrades to a warning, not an error.
        """
        import importlib.resources  # pylint: disable=import-outside-toplevel
