...
from nora_studio.commands import run as run_module
...
class TestNoraFleetRunner:
    """Tests for NoraFleetRunner"""

    @staticmethod
    def _make_runner() -> NoraFleetRunner:
        """Construct a NoraFleetRunner without invoking its heavy __init__."""
        return NoraFleetRunner.__new__(NoraFleetRunner)
...
class TestRunnerArgsInitialization:
    ...
    def test_cli_override_flips_mode_flag(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        ...
        monkeypatch.chdir(tmp_path)
        runner = NoraFleetRunner(cli_overrides={"server_only": True, "server_host": "example"})
